...
public class HotelCustomerDaoImpl implements HotelCustomerDao{
	...
	public List<Object> findHotelService(Map<String, Object> map) {
		...
		String sql00=" select d.department_name,count(d.department_name) depart,coalesce(sum(c.time_out_flag),0.00) timeout,coalesce(sum(c.use_time),0.00) amount_time, "
				+ "from "+sql0+" left join department_info d on aa.depart_id=d.department_id "
						+ "where aa.close_time between '"+ starttime +"' and '"+ endtime +"' group by d.department_name ";
		sql.append(sql00);
	...
	public List<Object> findDepartmentLoad(Map<String, Object> map) {
		...
		String sql0="((select ca.case_id,ca.author_name,ca.close_time,ca.depart_id,ca.use_time,ca.service_sort,ca.time_out_flag "
				...
				+ "where cs.customer_service_flag=1 and ca.depart_id='"+ depart +"') as a left join department_info d on a.depart_id=d.department_id ) as aa ";
		String sql1="select aa.author_name,st.Staff_no,count(ww.author_name) amount,coalesce(sum(ww.time_out_flag),0.00) timeout,coalesce(sum(ww.use_time),0.00) amount_time,aa.department_name from "+ sql0 +" left join staff_info st on aa.case_author=st.Staff_id where aa.close_time between '"+ starttime +"' and '"+ endtime +"' group by aa.author_name ";
		sql.append(sql1);
	...
	public List<Object> findRoomType(Map<String, Object> map) {
		...
		String sql0=" select aa.service_sort,count(aa.service_sort) amount,aa.given_time,coalesce(sum(aa.use_time),0.00) amount_time,coalesce(sum(aa.time_out_flag),0.00) timeout,d.department_name from "+ sql +" left join department_info d on aa.depart_id=d.department_id where aa.depart_id='"+ depart +"' and aa.close_time between '"+ starttime +"' and '"+ endtime +"' group by aa.service_sort ";
